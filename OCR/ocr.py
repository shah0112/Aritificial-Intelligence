...
- Emission Probability : Each pixel of the test and train images are compared to calculate the emission probabilities. The noise percentage was set 
...
# 
# Authors: (Shahidhya Ramachandran)
...
def initial_distribution(fname):
    ...
    for key, value in start_letter.items():
        if value != 0: 
            start_letter[key] = -np.log(value*1.0 / (no_lines+no_values)) 
        else:
            start_letter[key] = 20
    return start_letter
    ...
    for char1 in test_letters:
        ...
        for char2_cnt in range(len(train_letters)):
            ...
            for pixel_no in range(len(train_char)):
                if (train_char[pixel_no] == test_char[pixel_no]) and test_char[pixel_no] == '*':
                    emi_prob = 0.8
                elif (train_char[pixel_no] == test_char[pixel_no]) and test_char[pixel_no] == ' ':
                    emi_prob = 0.6
                elif train_char[pixel_no] == ' ':
                    emi_prob = 0.3
                elif train_char[pixel_no] == '*':
                    emi_prob = 0.3
